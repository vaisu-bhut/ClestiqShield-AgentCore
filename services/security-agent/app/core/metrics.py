...
class SecurityMetrics:
    ...
    def record_llm_tokens(self, input_tokens: int, output_tokens: int):
        ...
        logger.info("LLM tokens used", input=input_tokens, output=output_tokens, total=total)
    ...
    def record_request_end(self, blocked: bool, latency_ms: float, threat_score: float = 0.0):
        """Record request completion."""
        self.active_requests.add(-1)
        self.request_latency.record(latency_ms)
        self.threat_score_distribution.record(threat_score)
        
        if blocked:
            self.requests_blocked.add(1)
        else:
            self.requests_passed.add(1)
